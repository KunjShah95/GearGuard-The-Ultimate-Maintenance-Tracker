...
export function DocumentationPage() {
    ...
    return (
        <div className={`min-h-screen bg-slate-50 text-slate-600 selection:bg-primary/30 font-sans ${isPublic ? '' : 'pb-12 h-screen overflow-y-auto custom-scrollbar'}`}>
            {isPublic && <Navbar />}
            ...
            {!isPublic ? (
                ...
                <div className="pt-48 px-8 max-w-7xl mx-auto">
                    ...
                    <motion.h1
                        ...
                        className="text-6xl md:text-8xl font-display font-bold text-slate-900 tracking-tight mb-16 leading-[0.9]"
                    >
                        Knowledge <br />
            ...
            <div className={`px-8 ${isPublic ? 'max-w-7xl mx-auto mb-32' : 'mt-10'} space-y-12 animate-fade-in`}>
                {/* Hero Section */}
                <div className="relative p-12 bg-white rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden group">
                    ...
                    <div className="relative z-10 max-w-2xl">
                        <span className="section-number mb-6 block text-slate-400 font-mono">// DOCS_V4.2</span>
                        ...
                        <div className="flex gap-4">
                            ...
                            <button className="px-8 py-4 bg-slate-100 border border-slate-200 text-slate-500 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-colors">
                                Video Tutorials
                            </button>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {docSections.map((section, idx) => (
                        <motion.div
                            ...
                            className="bg-white border border-slate-200 rounded-3xl p-8 hover:border-primary/30 transition-all group shadow-sm hover:shadow-md"
                        >
                            ...
                            <h3 className="text-xl font-bold text-slate-900 mb-6 tracking-tight">{section.title}</h3>

                            <div className="space-y-6">
                                {section.items.map((item) => (
                                    <div key={item.label} className="group/item cursor-pointer">
                                        <div className="flex items-center justify-between mb-1">
                                            <h4 className="text-sm font-bold text-slate-700 group-hover/item:text-primary transition-colors">{item.label}</h4>
                                            <Icon icon="solar:arrow-right-up-linear" className="w-3 h-3 text-slate-400 group-hover/item:text-primary transition-colors" />
                                        </div>
                ...
                {/* Support Card */}
                <div className="p-8 bg-white border border-slate-200 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-8 shadow-sm">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h4 className="font-bold text-slate-900 text-lg">Need immediate tactical support?</h4>
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Our engineers are standing by for critical system assistance.</p>
                        </div>
                    </div>
                    <button className="px-10 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-primary hover:text-white transition-all shadow-lg hover:shadow-xl">
                        Open Support Ticket
                    </button>
                </div>
            </div>
