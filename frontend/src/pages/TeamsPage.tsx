            ...
            <div className="px-8 mt-8 space-y-8 animate-fade-in">
                ...
                <div className="flex flex-col lg:flex-row gap-4 items-center justify-between bg-surface-light/50 backdrop-blur-xl p-4 rounded-3xl border border-white/5 shadow-2xl">
                    ...
                    <Button className="h-12 rounded-2xl px-8 w-full lg:w-auto shadow-lg shadow-primary/20" onClick={() => setIsModalOpen(true)}>
                        <Icon icon="solar:users-group-rounded-bold" className="w-5 h-5 mr-2" />
                        Create Squad
                    </Button>
                </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    {filtered.map(team => (
                        <Card key={team.id} className="group relative overflow-hidden flex flex-col lg:flex-row p-0 bg-surface-light/30 backdrop-blur-md border-white/5 transition-all duration-500 hover:border-primary/30">
                            {/* Left Side: Team Branding */}
                            <div className="w-full lg:w-48 bg-zinc-900/40 border-b lg:border-b-0 lg:border-r border-white/5 p-8 flex flex-col items-center justify-center gap-4 text-center shrink-0">
                                <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-primary/20 to-indigo-500/10 flex items-center justify-center ring-4 ring-white/5 shadow-2xl group-hover:scale-110 transition-transform duration-500">
                                    <Icon icon="solar:shield-user-bold-duotone" className="w-10 h-10 text-primary" />
                                </div>
                                <div>
                                    <h4 className="font-black text-white text-lg tracking-tight">{team.name.split(' ')[0]}</h4>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-primary bg-primary/10 px-2 py-0.5 rounded-full border border-primary/20">Unit {team.id.padStart(2, '0')}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 p-8">
                                <div className="flex justify-between items-start mb-6">
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            <Icon icon="solar:case-minimalistic-bold-duotone" className="w-4 h-4 text-zinc-500" />
                                            <span className="text-xs font-bold text-zinc-500">{team.specialization}</span>
                                        </div>
                                    </div>
                                ...
                                <p className="text-sm text-zinc-400 leading-relaxed mb-8">{team.description}</p>
                                ...
                                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                                    <div className="flex -space-x-3">
                                        {team.members.map(member => (
                                            <div
                                                key={member.id}
                                                className="w-10 h-10 rounded-2xl flex items-center justify-center text-xs font-bold text-white border-2 border-zinc-900 shadow-xl relative group/member transition-transform hover:scale-110 hover:z-10"
                                                style={{ backgroundColor: stringToColor(member.user.name) }}
                                            >
                                                {getInitials(member.user.name)}
                                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-zinc-900 border border-white/10 rounded-xl text-[10px] font-bold text-white whitespace-nowrap opacity-0 group-hover/member:opacity-100 transition-all pointer-events-none shadow-2xl">
                                                    {member.user.name}
                                                    <span className="block text-[8px] text-zinc-500 uppercase tracking-widest">{member.role}</span>
                                                </div>
                                            </div>
                                        ))}
                                        <button className="w-10 h-10 rounded-2xl bg-zinc-900/50 border-2 border-dashed border-white/10 flex items-center justify-center hover:bg-primary/10 hover:border-primary/50 transition-all group/add">
                                            <Icon icon="solar:user-plus-bold" className="w-4 h-4 text-zinc-500 group-hover:text-primary" />
                                        </button>
                                    </div>

                                    <div className="flex items-center gap-6">
                                        <div className="text-right">
                                            <div className="text-xl font-black text-white">{team.equipmentCount}</div>
                                            <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">Assets</div>
                                        </div>
                                        <Button variant="outline" size="sm" className="h-10 rounded-xl px-4 text-xs font-bold border-white/5 group-hover:border-primary/50 group-hover:bg-primary/5">
                                            Details <Icon icon="solar:alt-arrow-right-bold" className="w-3.5 h-3.5 ml-2" />
                                        </Button>
                                    </div>
                                </div>
                            </div>
